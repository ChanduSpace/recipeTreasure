...
export default function EditRecipe() {
  ...
  useEffect(() => {
    const fetchRecipe = async () => {
      try {
        ...
        setExistingPhotoUrl(data.image || ""); // assuming backend returns image URL
      } catch (err) {
        console.error(
  ...
  const handleSubmit = async (e) => {
    ...
    if (photo) recipeData.append("image", photo); // only append if new photo uploaded
    ...
    try {
      await api.put(`/recipe/${id}`, recipeData, {
        ...
        >
          ...
          <label className="block w-full">
            ...
            <span className="bg-[#EFC81A] text-white py-2 px-4 block text-center rounded-lg cursor-pointer hover:bg-yellow-300">
              {photo || existingPhotoUrl ? "Change Photo" : "Upload Photo"}
            </span>
          </label>
